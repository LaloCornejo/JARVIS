...
class StreamingInterface:
    ...
    async def get_conversation_updates(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        while True:
            try:
                ...
                if stream is None:
                    break
                update = await asyncio.wait_for(stream.get(), timeout=1.0)
    ...
    async def get_transcription_updates(self) -> AsyncGenerator[str, None]:
        ...
        while True:
            try:
                stream = self.transcription_stream
                if stream is None:
                    break
                update = await asyncio.wait_for(stream.get(), timeout=1.0)
                if update is None:  # Stream closed
    ...
    async def push_tool_activity(self, tool_name: str, status: str, details: Dict[str, Any] | None = None):
