...
class SemanticMemory:
    ...
    def __init__(self, vector_memory: VectorMemory):
        ...
        self._decay_interval_hours = 24

    def _calculate_temporal_decay(self, created_at: str, last_accessed: str | None = None) -> float:
        """Calculate importance decay based on age."""
        try:
            created = datetime.fromisoformat(created_at)
            reference = datetime.fromisoformat(last_accessed) if last_accessed else datetime.now()
            age_hours = (reference - created).total_seconds() / 3600
            decay_factor = pow(self._decay_rate, age_hours / self._decay_interval_hours)
            return max(0.1, min(1.0, decay_factor))
        except (ValueError, TypeError):
            return 1.0

    async def boost_memory(self, memory_id: str) -> bool:
        """Boost importance of a memory when it's referenced."""
        if memory_id in self._memory_importance_scores:
            self._memory_importance_scores[memory_id] = min(
                1.0, self._memory_importance_scores[memory_id] + 0.2
            )
            return True
        return False
...
class EnhancedMemorySystem:
    ...
    async def clear_all(self):
        ...
        self.semantic_memory._memory_importance_scores.clear()
        self.semantic_memory._memory_clusters.clear()
        log.info("Enhanced memory system cleared")
