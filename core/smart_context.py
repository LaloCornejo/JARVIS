...
class SmartContextManager:
    ...
    async def _get_semantic_memories(self, query: str) -> list[dict]:
        try:
            ...
            if results:
                scores = [(r.get("text", "")[:50], r.get("score", 0)) for r in results]
                log.warning(f"[MEMORY] Search '{query[:30]}...' returned {len(results)} results, scores: {scores[:3]}")

            relevant = [r for r in results if r.get("score", 0) >= self.similarity_threshold]

            log.warning(f"[MEMORY] Found {len(relevant)}/{len(results)} relevant memories (threshold={self.similarity_threshold})")
            return relevant

        except Exception as e:
            log.warning(f"[MEMORY] Error getting semantic memories: {e}")
            return []
