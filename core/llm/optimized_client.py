...
class ContextSummarizer:
    ...
    async def summarize(self, messages: List[Dict[str, Any]], max_length: int | None = None) -> str:
...
class OptimizedLLMClient:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            ...
            elif self.backend == "openrouter" and self.api_key:
                headers["Authorization"] = f"Bearer {self.api_key}"
    ...
    async def chat(
        self, messages: list, system: str | None = None, tools: list | None = None
    ) -> AsyncIterator[dict]:
    ...
    async def chat_streaming(
        ...
        stream_callback: Callable[[dict], Coroutine[Any, Any, None]] | None = None,
        ...
        try:
            ...
            log.debug(f"[{prefix}] Request payload: {json.dumps(request_data, indent=2)[:9000]}")
            ...
            async with client.stream(
                ...
                if self.backend in ("nvidia", "openrouter"):
                    # Handle SSE format for NVIDIA/OpenRouter API
                    async for line in response.aiter_lines():
                        ...
                        try:
                            ...
                            if parsed_data.get("done"):
                                log.debug(f"[{self._log_prefix} SSE] Stream completed")
                                break

                            if "error" in parsed_data:
                                log.debug(
                                    f"[{self._log_prefix} SSE] Error in stream: {parsed_data['error']}"
                                )
                            ...
                            if "message" in parsed_data:
                                ...
                                log.debug(f"[{self._log_prefix} SSE] Message received: {message}")

                                # Handle content streaming
                                if "content" in message and message["content"]:
                                    content = message["content"]
                                    log.debug(f"[{self._log_prefix} SSE] Content chunk: {content}")
                                    buffer += content
                                    full_response += content  # Accumulate immediately
                                    log.debug(
                                        f"[{self._log_prefix} SSE] Buffer size: {len(buffer)}, Full response size: {len(full_response)}"
                                    )
                                    chunk_count += 1
                                ...
                                if should_yield:
                                    ...
                                    yield chunk_data
                                    log.debug(
                                        f"[{self._log_prefix} SSE] Yielded chunk {chunk_count}, cleared buffer"
                                    )
                                    buffer = ""  # Clear buffer after yielding
                        except Exception as e:
    ...
    def _prepare_nvidia_request(
        self, messages: list, system: str | None, tools: list | None, context_window: int | None
    ) -> dict:
        ...
        if "/" not in model_identifier:
            ...
            if "kimi" in model_identifier.lower():
                ...
                log.warning(
                    f"[{self._log_prefix}] Model '{model_identifier}' may not have correct format. Expected 'provider/model-name'"
                )

        log.debug(f"[{self._log_prefix}] Using model: {model_to_use}")
        ...
        try:
            ...
            log.debug(
                f"[{self._log_prefix}] Request payload validation: OK ({len(test_json)} bytes)"
            )
        except Exception as e:
            log.error(f"[{self._log_prefix}] Request payload validation FAILED: {e}")
            for key, value in request_data.items():
                try:
                    json.dumps(value)
                except Exception as field_error:
                    log.error(
                        f"[{self._log_prefix}] Problematic field '{key}': {field_error} - value type: {type(value)}"
                    )
    ...
    async def _optimize_context(self, messages: list, max_context: int | None = None) -> list:
    ...
    def _generate_cache_key(self, messages: list, system: str | None = None, tools: list | None = None) -> str:
    ...
    async def _stream_cached_response(
        self, cached_response: str, callback: Callable[[dict], Coroutine[Any, Any, None]] | None = None
    ) -> AsyncIterator[dict]:
