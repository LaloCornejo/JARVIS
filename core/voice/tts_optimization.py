                            ...
                            if len(current_part) + len(word) + 1 > self.max_chunk_size:
                                if current_part:
                                    ...
                                    if len(current_chunk.strip()) >= self.min_chunk_size:
                                        ...
                                        log.debug(f"Yielded sentence-part chunk (len:{len(current_chunk.strip())})")
                                        current_chunk = ""
                                current_part = word + " "
                            else:
                                current_part += word + " "
...
async def optimize_tts_streaming(original_text: str, 
                                   min_chunk_size: int = 50,
    ...
    try:
        ...
        return [original_text[i:i+max_chunk_size] for i in range(0, len(original_text), max_chunk_size)]
