...
class StreamingSpeechToText(OptimizedSpeechToText):
    ...
    async def start_streaming_transcription(
    ...
    ) -> AsyncIterator[str]:  # type: ignore[reportReturnType]
        ...
        try:
            async for audio_bytes in audio_stream:
                ...
                await self._process_streaming_chunk()  # type: ignore[reportGeneralTypeIssues]
    ...
    def _detect_voice_activity(self, audio_chunk: np.ndarray) -> bool:
        ...
        return bool(sustained_energy and voice_like)
    ...
    async def transcribe_streaming_file(
        ...
        try:
            ...
            if len(audio_data.shape) > 1:  # type: ignore[attr-defined]
                audio_data = np.mean(audio_data, axis=1)  # type: ignore[arg-type]
            ...
            for i in range(0, len(audio_data), chunk_samples):
                ...
                if len(chunk) > 0:
                    ...
                    chunk_bytes = chunk.astype(np.float32).tobytes()  # type: ignore[attr-defined]
                    yield chunk_bytes
