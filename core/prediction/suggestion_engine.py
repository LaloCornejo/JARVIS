...
class SmartSuggestionEngine:
    ...
    def _load_templates(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "browsing_docs": {
                ...
                "description": "You're reading documentation. Save this page for later?",
                "action": "bookmark_page",
                "action_params": {},
                "context_triggers": ["url_contains_docs"],
                "priority": SuggestionPriority.LOW,
            },
            "coding_debug": {
                "title": "Debugging Session",
                "description": "You're working on {open_file}. Need help debugging?",
                "action": "suggest_debug",
                "action_params": {},
                "context_triggers": ["is_coding"],
                "priority": SuggestionPriority.MEDIUM,
            },
            "terminal_activity": {
                "title": "Command Line Work",
                "description": "Working in terminal at {open_folder}. Save this session?",
                "action": "log_terminal_session",
                "action_params": {},
                "context_triggers": ["is_terminal"],
                "priority": SuggestionPriority.LOW,
            },
        }
    ...
    async def _should_trigger(self, template: Dict, context: SuggestionContext) -> bool:
        ...
        for trigger in triggers:
            if trigger == "time_of_day=morning" and context.time_of_day == "morning":
                ...
                return False
            elif trigger == "url_contains_docs" and context.url:
                doc_domains = ["docs.", "learn.", "tutorial", "readthedocs", "wiki.",
                               "stackoverflow", "dev.to", "medium.com"]
                if any(d in context.url.lower() for d in doc_domains):
                    return True
            elif trigger == "is_coding" and context.open_file:
                code_exts = (".py", ".js", ".ts", ".tsx", ".jsx", ".rs", ".go",
                            ".java", ".cpp", ".c", ".h", ".cs", ".rb", ".php")
                if any(context.open_file.endswith(ext) for ext in code_exts):
                    return True
            elif trigger == "is_terminal" and context.open_folder:
                terminal_apps = ("terminal", "powershell", "cmd", "wsl")
                if context.current_app and context.current_app.lower() in terminal_apps:
                    return True
    ...
    async def _create_suggestion_from_template(
        ...
        if "{open_file}" in description and context.open_file:
            description = description.replace("{open_file}", context.open_file)
        if "{open_folder}" in description and context.open_folder:
            description = description.replace("{open_folder}", context.open_folder)
