...
if not VOICE_AVAILABLE:
    log.warning("[DISCORD] Voice module not available: voice dependencies may not be installed")
...
class DiscordBotHandler:
    ...
    async def start(self, jarvis_server: "JarvisServer | None" = None) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._bot_user_id = self._bot_info.get("id") if self._bot_info else None
                self._bot_username = self._bot_info.get("username") if self._bot_info else None
        ...
        if isinstance(message, bytes):
            try:
                if message.endswith(self._zlib_suffix):
                    message = self._zlib_inflater.decompress(message) if self._zlib_inflater else message
            ...
            if VOICE_AVAILABLE and VoiceWebSocket:
                ...
                async def on_session_description(secret_key: bytes) -> None:
                    ...
                    if self._voice_player:
                        ...
                        log.warning(f"[DISCORD] No voice player available for guild {guild_id}")
                        if not connection_ready.done():
                            connection_ready.set_result(True)
    ...
    async def _process_message_through_jarvis(
        ...
        typing_task = asyncio.ensure_future(self._keep_typing(channel_id))
        if message_id:
        ...
        try:
            ...
            if full_response:
                ...
                if thinking_text and session.thinking_enabled:
                    ...
                    if message_id:
                        await self._remove_own_reaction(channel_id, message_id, "🗿")
                        await self._add_reaction(channel_id, message_id, "💭")
                ...
                if tts_available:
                    ...
                    if in_voice and self._voice_player and voice_player_connected:
                        ...
                        log.debug("[DISCORD] Not sending to voice - using audio attachment")
                        try:
    ...
    async def _add_reaction(self, channel_id: str, message_id: str, emoji: str) -> None:
        ...
        try:
            ...
            await self._http_client.put(
                f"{self.API_URL}/channels/{channel_id}/messages/{message_id}/reactions/{emoji_encoded}/@me",
            )
        except Exception:
            pass

    async def _remove_own_reaction(self, channel_id: str, message_id: str, emoji: str) -> None:
        """Remove bot's reaction emoji from a message."""
        if not self._http_client:
            return
        try:
            emoji_encoded = urllib.parse.quote(emoji, safe="")
            ...
            if transcription and len(transcription.strip()) > 2:
                ...
                if session:
                    await self._process_message_through_jarvis(
                        session, transcription, session.channel_id, message_id=""
                    )
        except Exception as e:
    ...
    async def _handle_thinking(
        ...
        if len(parts) > 1:
            ...
            if arg in ("on", "enable", "true", "1", "yes"):
                ...
                await self._send_message(
                    session.channel_id,
                    "Usage: `!thinking` (toggle) or `!thinking on` / `!thinking off`",
                )
                return
        else:
