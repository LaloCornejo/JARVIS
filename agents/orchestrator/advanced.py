...
Please provide additional insights or perspectives on this task."""
                ...
                supporting_results = [r for r in raw_results if isinstance(r, dict)]
    ...
    async def route_request(self, message: str) -> Dict[str, Any]:
        ...
        Wrapper around route_task for compatibility with assistant.py.
        Returns dict with use_agent, agent_type, and confidence keys.
        """
        decision = await self.route_task(message)

        return {
            "use_agent": decision.confidence >= 0.5,
    ...
    async def process_with_agent(
        self, agent_type: str, request: str, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            result = await agent.process(request, agent_context)
            return {
                "success": True,
                "response": result,
                "agent": agent_type,
