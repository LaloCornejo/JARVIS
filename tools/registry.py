...
class ReadToolTool(BaseTool):
    name = "read_tool"
    description = "Discover available tools. Call with no args to list all tools. Pass a tool name to get its description and parameters."
    parameters = {
        "type": "object",
        "properties": {
            "tool_name": {
                "type": "string",
                "description": "Tool name to inspect (omit to list all available)",
            },
        },
        "required": [],
    }
    ...
    async def execute(self, tool_name: str | None = None) -> ToolResult:
        ...
        if not tool_name:
            ...
            for n in sorted(names):
                prefix = n.split("_")[0] if "_" in n else ""
        ...
        if not tool:
            return ToolResult(
                ...
                error=f"Tool '{tool_name}' not found. Call read_tool() with no args to list all.",
            )
        schema = tool.to_schema()
        import json
        return ToolResult(success=True, data=json.dumps(schema, indent=2))


class RunToolTool(BaseTool):
    name = "run_tool"
    description = "Execute any tool by name with its parameters. Use read_tool() first to discover tool names and expected parameters."
    parameters = {
        "type": "object",
        "properties": {
            "tool_name": {
    ...
    async def execute(self, tool_name: str, params: dict[str, Any] | None = None, **kwargs: Any) -> ToolResult:
        if tool_name == "run_tool":
            return ToolResult(
                success=False, data=None,
                error="run_tool cannot call itself. Use the tool directly instead.",
            )
        reg = get_tool_registry()
        tool = reg.get(tool_name)
        if not tool:
            return ToolResult(
                success=False, data=None,
                error=f"Tool '{tool_name}' not found. Use read_tool() to list available tools.",
            )
...
from tools.integrations import (
    # Calendar tools - disabled, using gws CLI instead
    ...
    GitHubSearchTool,
    GwsCalendarAgendaTool,
    GwsCalendarEventTool,
    GwsGmailListTool,
    GwsGmailSendTool,
    GwsTool,
    ...
    # Gmail tools - disabled, using gws CLI instead
    # GmailArchiveTool,
    # GmailDeleteTool,
    # GmailListTool,
    # GmailMarkReadTool,
...
from tools.skills import ReadSkillTool
...
TOOL_CATEGORIES = {
    ...
    "homeassistant": {
        "tools": [
            ...
            "homeassistant_toggle",
            "homeassistant_get_services",
...
class ToolRegistry:
    ...
    def get_filtered_schemas(self, query: str, max_tools: int = 15) -> list[dict]:
        query_lower = query.lower()

        # If user is asking about tools/capabilities, return max_tools schemas
        # (enough for LLM tool-calling but capped so small models don't choke).
        tool_query_kw = [
            "what tools", "list tools", "all tools", "tools you have",
            "what are your", "your tools", "list of tools", "available tools",
            "what can you do", "capabilities", "what are you capable",
            "give me the list", "list of all", "tools do you have",
            "what are all", "show me what", "what do you have",
            "tools available", "tools do you use", "which tools",
        ]
        if any(kw in query_lower for kw in tool_query_kw):
            return self.get_schemas()[:max_tools]
