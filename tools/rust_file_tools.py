...
from core.rust_tools import async_file_search
    ...
    async def execute(
        ...
        try:
            result = await async_file_search(
                pattern=pattern, path=path, limit=limit, ignore_case=ignore_case
            )
            ...
            if result["success"]:
                ...
                return ToolResult(
                    success=False, data=None, error=result.get("error", "Unknown error")
                )

        except Exception as e :
            print(f"Error occurred while executing RustFileSearchTool: {e}")
