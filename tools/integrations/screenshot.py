...
class ScreenshotCaptureTool(BaseTool):
    ...
    description = (
        "Save a screenshot to a file. Returns the file path only — does NOT read or describe the image. "
        "NEVER use this if the user wants to know what's on screen. "
        "For describing what's on screen, call screenshot_analyze — that's what it's for."
    )
    parameters = {
        "type": "object",
        "properties": {
...
class ScreenshotAnalyzeTool(BaseTool):
    ...
    async def execute(
        ...
        try:
            ...
            log.info("Preloading vision model...")
            preload_success = await vision.preload_model()
            log.info(f"Vision model preload result: {preload_success}")
