...
class AgentExecuteTool(BaseTool):
    ...
    async def execute(
        self, agent_type: str, request: str, context: dict[str, Any] | None = None
    ) -> ToolResult:
        try:
...
class CreateWorkflowTool(BaseTool):
    ...
    async def execute(
        self, name: str, trigger_type: str, trigger_config: dict[str, Any], actions: list[str]
    ) -> ToolResult:
        try:
    ...
    async def execute(self) -> ToolResult:
        try:
            ...
            for trigger_dict in triggers:
                if trigger_dict.get("trigger_id", "").startswith("workflow_"):
                    workflows.append(
                        {
                            "id": trigger_dict["trigger_id"],
                            "enabled": trigger_dict.get("status") == "active",
                            "trigger_count": trigger_dict.get("trigger_count", 0),
                            "last_triggered": trigger_dict.get("last_triggered"),
                        }
                    )
...
class CreateUserTool(BaseTool):
    ...
    async def execute(
        self, name: str, permission_level: str = "user", preferences: dict[str, Any] | None = None
    ) -> ToolResult:
        try:
            from pathlib import Path

            from core.multi_user.user_manager import PermissionLevel, UserManager, UserPreference
            ...
            user_id = await user_manager.create_user(
                ...
                preferences=UserPreference.from_dict(preferences) if preferences else None,
            )
...
class GetSuggestionsTool(BaseTool):
    ...
    async def execute(self, user_id: str, context: dict[str, Any] | None = None) -> ToolResult:
        try:
            from core.prediction.suggestion_engine import SmartSuggestionEngine, SuggestionContext
            ...
            suggestion_context = SuggestionContext(
                ...
                system_load=ctx.get("system_load"),
