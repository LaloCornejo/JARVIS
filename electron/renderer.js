...
document.addEventListener("DOMContentLoaded", () => {
  ...
  ipcRenderer.on("mouse-position", (event, pos) => {
    ...
    console.log(`[Mouse] abs: ${pos.x},${pos.y} win: ${pos.windowX},${pos.windowY} rel: ${relX},${relY}`);
    
    targetX = normX;
    targetY = normY;
    
    const modelLeft = 2675;
    const modelRight = 3071;
    const modelTop = 880;
    const modelBottom = 1400;
    
    const isOverModel = relX >= modelLeft && relX <= modelRight && 
                        relY >= modelTop && relY <= modelBottom;
    
    if (jarvis.live2dManager.targetOpacity !== undefined) {
      jarvis.live2dManager.targetOpacity = isOverModel ? 0 : 1;
    }
    model.alpha = isOverModel ? 0 : 1;
